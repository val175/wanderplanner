...
- **No Horizontal Scroll**: Adding or modifying columns must **never** introduce a horizontal scrollbar. For `table-fixed` tables, the `<th>` header width is the source of truth — it must always match the `<td>` body width. Action columns containing pencil+trash icons must use `w-[80px]` (custom `<tr>` tables) or `size: 72` (TanStack Table). Never hardcode `w-10` on action `<td>` elements — this overrides the column `size` and clips content.
