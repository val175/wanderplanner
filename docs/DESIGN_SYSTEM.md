...
For dropdowns:
- **Use the shared Select**: `src/components/shared/Select.jsx` (Radix Select). Do not use native `<select>`.
- **Popover styling**: Must match `DatePicker` (bg-bg-card, border-border, radius-md, no shadows).
- **Trigger styling**: Same input tone as DatePicker (bg-bg-card, border-border, rounded-md, hover bg-bg-hover/50).
- **Status menus**: Use inline color dots in items for quick scanning; no section headers.
...
- **Container**: Wrap the table in a `<Card className="overflow-hidden">` with an inner `<div className="overflow-x-auto scrollbar-thin">`. The Card provides the border, background, and the outer breathing room.
- **Surface Borders**: All table/kanban/calendar primary surfaces must use `border border-border` (never `/10` or `/50`) so tabs share the same visual weight.
...
- **No Horizontal Scroll**: Adding or modifying columns must **never** introduce a horizontal scrollbar. For `table-fixed` tables, the **`<th>` header width is the binding constraint** — the browser locks every column to the header width and clips any `<td>` content that exceeds it. The `<td>` width attribute is only a hint in this mode. **Both `<th>` and `<td>` must use the same width value.** Action columns containing icons (pencil, trash, toggle) must declare `w-[80px]` on both the `<th>` and `<td>` (custom table), or `size: 72` (TanStack Table column def). For flex-based list layouts (e.g. TodoTab), the header spacer `<div>` must match the corresponding item action `<div>` width exactly. Never use `w-10` or `w-[40px]` for any action column that holds two icons — minimum is `w-[80px]`.
