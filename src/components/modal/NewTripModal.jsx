...
function WandaChatFlow({ onPlanReady, onBack }) {
  ...
  return (
    <div className="flex flex-col h-[500px] animate-fade-in">
      {/* Header — pr-10 keeps the counter clear of the absolute close button */}
      ...
      {/* Vibe picker — multi-select pills */}
      {showVibePicker && (
        <div className="border-t border-border px-4 py-3 shrink-0 space-y-2.5">
          <div className="flex flex-wrap gap-2">
            {VIBE_OPTIONS.map(v => {
      ...
      {/* Budget picker — single-select range pills */}
      {showBudgetPicker && (
        <div className="border-t border-border px-4 py-3 shrink-0 space-y-2.5">
          <div className="flex flex-wrap gap-2">
      ...
      {/* Text input — shown for all non-picker questions */}
      {showInput && (
