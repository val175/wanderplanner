...
export const TODO_STATUS_ORDER = ['not_started', 'in_progress', 'done']
export const getTodoStatus = (todo) => todo.status || (todo.done ? 'done' : 'not_started')

export function isPastDue(isoDateStr) {
...
// Anchors the DragOverlay to the cursor
export const snapCursorToTopLeft = ({ activatorEvent, draggingNodeRect, transform }) => {
...
export function getDeepLinkTarget(text) {
...
export function TodoStatusSelect({ value, onChange, disabled }) {
...
export function TodoStatusBadge({ value }) {
  const current = TODO_STATUSES.find(s => s.id === value) || TODO_STATUSES[0]
...
export function AssigneePill({ value, onChange, tripTravelers, resolveProfile, currentUserProfile }) {
...
export function TodoMobileCard({ todo, onDelete, onClick, resolveProfile, tripTravelers, currentUserProfile, isReadOnly, onUpdate }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-bg-card border border-border p-3 rounded-[var(--radius-md)] cursor-pointer transition-all ${status === 'done' ? 'opacity-60' : ''}`}
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <span className={`text-[13px] font-semibold flex-1 leading-snug ${status === 'done' ? 'line-through text-text-muted' : 'text-text-primary'}`}>
          {todo.text}
        </span>
    ...
    return (
      ...
      >
        <div className="flex items-start gap-2">
          ...
          <span className={`text-[13px] font-medium leading-snug flex-1 transition-all duration-200 ${status === 'done' ? 'line-through text-text-muted' : 'text-text-primary'}`}>
            {todo.text}
          </span>
        </div>
    ...
    >
      <div className="flex items-center gap-2 py-3">
        <div className="flex-1 min-w-0 flex items-center gap-2 px-2">
          ...
          <span className={`text-[14px] font-medium transition-all duration-200 ${status === 'done' ? 'line-through text-text-muted' : 'text-text-primary'}`}>
            {todo.text}
          </span>
        ...
        <div className={`w-[140px] shrink-0 flex justify-start px-2 transition-opacity duration-150 ${!todo.dueDate ? 'opacity-0 group-hover:opacity-100' : ''}`} onClick={e => e.stopPropagation()}>
          <DatePicker
            value={todo.dueDate}
        ...
        {!isReadOnly && (
          <div className="w-[60px] shrink-0 flex items-center justify-end gap-1 pr-2">
            ...
            <button
              onClick={(e) => { e.stopPropagation(); triggerHaptic('medium'); onDelete() }}
              className="opacity-0 group-hover:opacity-100 scale-90 group-hover:scale-100 blur-sm group-hover:blur-none transition-all duration-150 ease-out p-1 text-text-muted hover:text-danger rounded-[var(--radius-sm)]"
              title="Delete task"
            >
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>
            </button>
...
export function SortableTodoItem(props) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({
    id: props.todo.id,
    data: { type: 'Todo', todo: props.todo },
...
// Bug fix: onTodoClick is now passed as a prop instead of referencing a parent-scope variable
export function BoardStatusColumn({ status, statusTodos, canDrag, isReadOnly, dispatch, handleToggle, handleStatusChange, handleDeepLink, onTodoClick, resolveProfile, tripTravelers, currentUserProfile }) {
  ...
  return (
    <div ref={setNodeRef} className={`flex flex-col flex-shrink-0 w-72 bg-bg-secondary/20 border rounded-[var(--radius-lg)] p-2 transition-colors ${isOver ? 'border-accent/50 bg-accent/5' : 'border-border'}`}>
      <div className="px-3 py-2 mb-2 flex items-center justify-between border-b border-border/30">
        <h3 className="font-semibold text-sm text-text-primary">{status.label}</h3>
