...
export default function TodoDrawer({ todo, travelers, onUpdate, onAddComment, onUpdateComment, onDeleteComment, onClose, resolveProfile, isReadOnly }) {
  ...
  return createPortal(
    <div className="relative z-[9999] font-heading">
      ...
      {/* Drawer */}
      <div
        className={`fixed w-full flex flex-col bg-bg-card border-border transform transition-transform duration-300 ease-out inset-x-0 bottom-0 rounded-t-2xl min-h-[92dvh] max-h-[92dvh] border-t md:inset-y-0 md:right-0 md:left-auto md:max-w-xl md:rounded-none md:min-h-0 md:max-h-none md:border-t-0 md:border-l ${mounted ? 'translate-y-0 md:translate-x-0' : 'translate-y-full md:translate-y-0 md:translate-x-full'}`}
      >
        {/* Mobile drag handle */}
