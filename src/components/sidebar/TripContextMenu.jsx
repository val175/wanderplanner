...
export default function TripContextMenu({ tripId, tripName, onClose }) {
  ...
  // Find the actual trip object for the share modal
  ...
  }, [onClose, showDeleteConfirm, showShareModal])
  ...
  const handleUnarchive = (e) => {
    e.stopPropagation()
    dispatch({ type: ACTIONS.UNARCHIVE_TRIP, payload: tripId })
  ...
  const menuItems = [
    ...
    ...(isReadOnly ? [
      ...
      ...(effectiveStatus === 'archived' ? [{
        label: 'Restore Trip',
        icon: (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="1 4 1 10 7 10" /><path d="M3.51 15a9 9 0 1 0 .49-3.5" /></svg>),
  ...
  return (
    <>
      ...
      {showSettleUpModal && (
        <SettleUpModal
          tripId={tripId}
          onClose={() => { setShowSettleUpModal(false); onClose() }}
        />
      )}
    </>
  )
